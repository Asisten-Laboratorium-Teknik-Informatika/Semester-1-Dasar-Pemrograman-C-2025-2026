/* Kelompok 4 - Program Pelayanan RS - Medicare X
 ...
 Perbaikan:
 - rapikan menu sesuai "Rancangan Medicare X" (menu utama + tindakan 02/04)
 - submenu Tes: 01=Tes Narkoba (Rp100.000), 02=Cek Kesehatan (Rp200.000)
 - hanya pasien Mandiri boleh melakukan tes ini (BPJS ditolak)
 - input NIK/No.Identitas ditambahkan pada pendaftaran dan dicetak di surat
 - langsung lanjut otomatis ke apotek/kamar bila keputusan dokter pilihannya mengarah ke sana
*/
...
int main() {
    ...
    while (1) {
        ...
        int opt = atoi(buf);
        if (opt == 0) {
            printf("Selesai. Sampai jumpa — semoga tetap sehat.\n");
            return 0;
        }
        switch (opt) {
