...
void loadingAnimation(const char *text, int dotCount, int delayMs) {
    printf("%s", text);
...
int main() {
    ...
    if (total == 0){
        printf("Tidak ada soal yang diload. Periksa soal.txt\n");
        return 1;
    }
...
login:
    ...
    system("cls");
    loadingAnimation("Memverifikasi akun", 5, 250);
...
kategori:
    do {
        system("cls");
        loadingAnimation("Memuat kategori", 4, 200);
        system("cls");
        printf("\n\n=== QUIZ GAME ===\n\n");
        ...
        if (count == 0) {
            printf("Kategori kosong! Pilih lagi...\n");
            Sleep(1200);
            goto kategori;
        }

        system("cls");
        loadingAnimation("Menyiapkan soal", 5, 200);
        system("cls");
